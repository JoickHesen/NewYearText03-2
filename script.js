...
export function mySpark() {
	...
	const randomWords = ["黄裕惠", "新年快乐", "心想事成", "黄裕惠", "黄裕惠", "黄裕惠", "万事如意"];
	const wordDotsMap = {};
	randomWords.forEach((word) => {
		//wordDotsMap[word] = MyMath1.literalLattice(word, 3, "Gabriola,华文琥珀", "90px");
		wordDotsMap[word] = MyMath.literalLattice(word, 300, "Gabriola,楷体", "90px");
	});
	...
	function getRandomShellPositionH() {
		...
		//return fitShellPositionInBoundsH(1);
	}
	...
	function seqPyramid(myNeedShell1, myNeedShell2) {
		const barrageCountHalf = IS_DESKTOP ? 7 : 4;
		const largeSize = shellSizeSelector();
		const smallSize = Math.max(0, largeSize - 3);
		const randomMainShell = Math.random() < 0.78 ? myNeedShell1 : myNeedShell2;
	...
	function seqSmallBarrage(myNeedShell1, myNeedShell2) {
	...
	function startSequence() {
		...
		if (rand == 1) {
			//rand = Math.random();
			rand = 2;
			return seqPyramid(crackleShell, crackleShell);
			//return seqPyramid(crackleShell);
		...
		if (rand == 2) {
			rand = 3;
			return seqSmallBarrage(fallingLeavesShell, fallingLeavesShell);
		}
		if (rand == 3) {
			rand = 4;
			return seqPyramid(ghostShell, ghostShell);
		}
		if (rand == 4) {
			rand = 5;
			return seqPyramid(floralShell, floralEffect);
		}
		else
			rand = Math.random();
	...
	function ChangeAutoEvent(event){
		//console.log(store.state.config.autoLaunch);
		store.state.config.autoLaunch = true;
	}
	document.addEventListener('mousedown',ChangeAutoEvent);
	...
	// 	autoLaunch = true;
	// }
	// document.addEventListener('click', autoChange);
	...
	function getWordDots(word) {
		...
		var res = MyMath.literalLattice(word, 3, "Gabriola,楷体", fontSize + "px");
